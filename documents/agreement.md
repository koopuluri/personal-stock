...
The owner, each shareholder, and each prospective shareholder must maintain or provide a current electronic notice address in the private supporting records or applicable adoption record. Unless another method is required by law or agreed for a particular transaction, electronic delivery and execution are the primary means of giving any notice, adoption, approval, consent, or other writing under this agreement. A person electronically signs or approves a record by replying from that person's electronic notice address to a message that provides the complete record in retainable form and identifies it by document or proposal ID and, when applicable, agreement version and content hash, with an unambiguous statement of assent, or by completing another electronic process that reasonably authenticates the person and records the same intent. A reply stating "I agree" is sufficient when the message states the legal effect of that reply; no typed name in the reply or signature on the underlying document is required. That action constitutes the person's writing and electronic signature, signifies consent to conduct that transaction electronically, and is effective when received in retrievable form. The owner must retain the record, assent, and reasonably available attribution, delivery, and timestamp evidence in the private supporting records; known delivery failure requires another reasonable method.
