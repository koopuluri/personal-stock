...
A “published agreement version” is a complete version of this agreement made available in a publicly accessible source repository under a stated version label. It is identified by that version label and its `AGREEMENT_CONTENT_HASH`.

`AGREEMENT_CONTENT_HASH` is the Keccak-256 hash of the exact UTF-8 bytes of the published agreement source file, computed without normalization or transformation. A repository URL, commit identifier, release tag, filename, or other source reference facilitates retrieval but is not part of the agreement version's authoritative identity. If a version label, source reference, or reproduction conflicts with the content hash, the exact content matching the content hash controls.

A person “adopts” this agreement for a personal stock by signing a record that identifies the personal stock by its blockchain chain ID and authoritative stock-contract address and identifies this agreement by its version and `AGREEMENT_CONTENT_HASH`. Before signing, the person must receive the complete agreement content identified by that content hash, or be given access to it in a retainable form. An adoption applies only to the personal stock it identifies.
...
The owner must preserve the exact source bytes of every agreement version that governs or formerly governed the personal stock and every replacement version formally proposed under §12. The owner must ensure that each such published agreement version remains available in at least one publicly accessible source repository together with its version, `AGREEMENT_CONTENT_HASH`, and available source references. The owner must also provide the exact source bytes directly to any shareholder on request.

Temporary loss or unavailability of a repository, URL, tag, or other source reference does not change the governing agreement, invalidate an adoption, or alter any person's rights or obligations. The owner must restore public availability within a reasonable time after learning of the loss or unavailability.
...
The owner, each shareholder, and each prospective shareholder must maintain or provide a current electronic notice address in the private supporting records or applicable adoption record. Unless another method is required by law or agreed for a particular transaction, electronic delivery and execution are the primary means of giving any notice, adoption, approval, consent, or other writing under this agreement. A person electronically signs or approves a record by sending a message, or replying to one, from that person's electronic notice address when the message provides or is logically associated with the complete record in retainable form, sufficiently identifies the matter and, when applicable, the agreement version and content hash, and unambiguously states the effect of assent, or by completing another electronic process that reasonably authenticates the person and records the same intent. An initiating message constitutes the sender's electronic signature if it states that intent. A reply stating "I agree" is sufficient when the message states the legal effect of that reply; no typed name in the reply or signature on the underlying document is required. That action constitutes the person's writing and electronic signature, signifies consent to conduct that transaction electronically, and is effective when received in retrievable form. The owner must retain the record, assent, and reasonably available attribution, delivery, and timestamp evidence in the private supporting records; known delivery failure requires another reasonable method.
...
This agreement is the complete statement of the terms governing the personal stock. A signed adoption, award, transaction document, summary, or other record may identify the personal stock, the parties, and the applicable published agreement; reproduce this agreement for readability; and establish facts or separate obligations, but it changes this agreement only through §12. The authoritative text is the exact agreement content identified by the version and `AGREEMENT_CONTENT_HASH` stated in the applicable signed adoption or effective amendment record. A repository's default branch, a later commit, a moved or reused release tag, a rendered PDF, or any other reproduction does not modify that text. If any source reference or reproduction differs from the exact content identified by the content hash, the exact content identified by the hash controls.
